# app/services/simulations/game_from_player_means.rb
require "zlib"

module Simulations
  class GameFromPlayerMeans
    MODEL_VERSION = "game_from_player_means_v1".freeze
    ...
    # If sims > 0, we run a game Monte Carlo using player means/sds from ProjectionDistribution.
    # If sims <= 0, we save a fast "means only" row.
    def build!(game_id:, sims: 0, persist: true)
      ...
      if sims_i > 0
        build_mc!(game: game, home_rows: home_rows, vis_rows: vis_rows, sims: sims_i, persist: persist)
      else
        build_means_only!(game: game, home_rows: home_rows, vis_rows: vis_rows, persist: persist)
      end
    end

    private

    def build_means_only!(game:, home_rows:, vis_rows:, persist:)
      ...
      payload = {
        ...
        home_points_sd: 0.0,
        visitor_points_sd: 0.0,
        win_prob_home: nil,

        spread_mean: (home_mean - vis_mean),
        spread_sd: 0.0,
        spread_p10: nil, spread_p50: nil, spread_p90: nil,

        total_mean: (home_mean + vis_mean),
        total_sd: 0.0,
        total_p10: nil, total_p50: nil, total_p90: nil,
    ...
    def build_mc!(game:, home_rows:, vis_rows:, sims:, persist:)
      home_pts = []
      vis_pts  = []
      spreads  = []
      totals   = []
      ...
      sims.times do |i|
        h = simulate_team_points(home_rows, seed_base: "#{game.id}-home-#{i}")
        v = simulate_team_points(vis_rows,  seed_base: "#{game.id}-vis-#{i}")
      ...
      payload = {
        ...
        win_prob_home: (home_wins.to_f / sims.to_f),
    ...
    def save_distribution!(payload)
      ...
      gs.assign_attributes(
        ...
        home_points: payload[:home_points_mean].to_f.round,
        visitor_points: payload[:visitor_points_mean].to_f.round,
