...
module ::Projections
  class DvpOnlyModel
    MODEL_VERSION = "dvp_only_v1".freeze
    ...
    def project_stats(inputs)
      ...
      # 💡 dampen effect on points
      pts_adj_mult = 1.0 + ((pts_mult - 1.0) * 0.5)
      reb_adj_mult = reb_mult
      ast_adj_mult = ast_mult

      puts "────────────────────────────────────────────"
      puts "[DVP] #{inputs[:player].name} (#{pos}) vs #{opponent.abbreviation}"
      puts "Ranks   →  Pts: #{pts_rank&.round(1)}  Reb: #{reb_rank&.round(1)}  Ast: #{ast_rank&.round(1)}"
      puts "Effects →  Pts: #{pts_label} (adj #{pts_adj_mult.round(3)})  Reb: #{reb_label}  Ast: #{ast_label}"
      puts "L5 Avg  →  Pts: #{b[:points].round(1)}  Reb: #{b[:rebounds].round(1)}  Ast: #{b[:assists].round(1)}"
      puts "Proj    →  Pts: #{(b[:points] * pts_adj_mult).round(2)}  Reb: #{(b[:rebounds] * reb_adj_mult).round(2)}  Ast: #{(b[:assists] * ast_adj_mult).round(2)}"
      puts "────────────────────────────────────────────"

      {
        points:   (b[:points]   * pts_adj_mult).round(2),
        rebounds: (b[:rebounds] * reb_adj_mult).round(2),
        assists:  (b[:assists]  * ast_adj_mult).round(2)
      }
    end
